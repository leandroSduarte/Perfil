...
#DEFINE COLCAB1			002
#DEFINE COLINF1			110
#DEFINE ITEMP1			013   // Quantidade de itens que podem ser impressos na página 1 + rodapé
#DEFINE MAXITP1			019   // Máximo de itens que podem ser impressos na página 1
...
Static Function PrtCabec(oPDF)
	...
	oPDF:Say(nLinAtu+=VSPACE	, nColAtu+620, "São Paulo, " + cValToChar(Day(dDatabase)) + " de " + MesExtenso(Month(dDatabase)) + " de " + cValToChar(Year(dDatabase)), oFont16:oFont)
	oPDF:Say(nLinAtu+=VSPACE	, nColAtu+620, "Status do Pedido: Aberto", oFont16:oFont)
	oPDF:Say(nLinAtu+=VSPACE	, nColAtu+620, "Validade do Orçamento: 15 dias úteis", oFont16:oFont)
	nLinAtu := 110
	...
	oPDF:Say(nLinAtu+=VSPACE	, nColAtu+002, "Endereço", oFont16:oFont)
...
Static Function PrtMsg(oPDF)
	...
	oPDF:Say(nLinAtu+=VSPACE, nColAtu+002, "Conforme solicitação, apresentamos orçamento para o fornecimento dos itens abaixo discriminados, visando o atendimento de vossa solicitação", oFont16:oFont)
	oPDF:Say(nLinAtu+=VSPACE, nColAtu+002, "de acordo com as referências dos produtos, descrições, fabricantes/similiriadades (onde autorizados) e quantidade informadas.", oFont16:oFont)
...
Static Function PrtCabIt(oPDF)
	...
	oPDF:Say(nLinAtu	, nColAtu+046, "Código"			, oFont12N:oFont)
	oPDF:Say(nLinAtu	, nColAtu+118, "Referência"		, oFont12N:oFont)
	oPDF:Say(nLinAtu	, nColAtu+220, "Descrição"		, oFont12N:oFont)
		...
		While !AB5->(EoF()) .And. AB5->AB5_FILIAL + AB5->AB5_NUMORC + AB5->AB5_ITEM == AB4->AB4_FILIAL + AB4->AB4_NUMORC + _cItem
			...
			EndIf
				AAdd( aItens, {{ cSeq														, 006 },; //Sequência
							  ...
							  { AllTrim(AB5->AB5_DESPRO)									, 195 },; //Descrição
							  ...
							  { Transform(AB5->AB5_VUNIT, "999,999.99")						, 740 },; //Valor Unitário
					...
					AAdd(aItens, {{cSeq													, 006},; //Sequência
							  ...
							  {SB1->B1_UM												, 398},; //Unidade de Medida
							  {cValToChar(AB5->AB5_QUANT)								, 426},; //Quantidade
							  {Transform(AB5->AB5_VUNIT, "@E 999,999.99")					, 639},; //Valor Unitário
