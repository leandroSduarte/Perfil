...
//	_oPrint:FWMSBAR("CODE128" /*cTypeBar*/,nLinBar/*nRow*/ ,0.7/*nCol*/ ,cCode  /*cCode*/,_oPrint/*oPrint*/,/*lCheck*/,/*Color*/,/*lHorz*/, /*nWidth*/,1.0/*nHeigth*/,/*lBanner*/,/*cFont*/,/*cMode*/,.F./*lPrint*/,/*nPFWidth*/,/*nPFHeigth*/,/*lCmtr2Pix*/)
	...
	While SD4->(!Eof()) .And. D4_FILIAL+D4_OP == cSeekSD4
		...
		If SD4->D4_QUANT > 0 
			...
			If Len(Alltrim(SB1->B1_DESC)) > 40
				...
				AADD(aEmpenhos, {SD4->D4_COD,Subs(Alltrim(SB1->B1_DESC),1,40),""      ,SD4->D4_QUANT,SB1->B1_UM,SD4->D4_LOCAL,NNR->NNR_DESCRI ,SD4->D4_LOTECTL,_cNumSeri})
				AADD(aEmpenhos, {"",Subs(Alltrim(SB1->B1_DESC),41,10),"",,"","","","",""})
			Else
				//               D4_COD     ,B1_DESC              ,COR     ,D4_QUANT     ,B1_UM      ,D4_LOCAL    ,NNR_DESCRI      ,D4_LOTECTL     ,SERIE
				AADD(aEmpenhos, {SD4->D4_COD,Alltrim(SB1->B1_DESC),""      ,SD4->D4_QUANT,SB1->B1_UM,SD4->D4_LOCAL,NNR->NNR_DESCRI ,SD4->D4_LOTECTL,_cNumSeri})			
			Endif
	...
	If Len(aEmpenhos) > 0 
		...
		For nY := 1 to Len(aEmpenhos)
			...
			_cPrtLin += PADR(aEmpenhos[nY][7],22) //Desc.Amz
