...
ฑฑบPrograma  | DDMAX11X   บAutor  ณ Gabriel Verํssimo บ Data ณ  11/07/19  บฑฑ
ฑฑฬออออออออออุออออออออออออสอออออออฯออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Rotina de integra็ใo de Grupos MOBILE                      บฑฑ
...
User Function DDMAX11X(lMensagem, lDel)
	...
	Local c_key			:= getNewPar("MV_XCHTASK","")
	...
	If !Empty(SBM->BM_XMAXID)
		c_Url	:=	"https://api.umov.me/CenterWeb/api/"+AllTrim(c_key)+"/subGroup/"+AllTrim(SBM->BM_XMAXID)+".xml"
	Else
		c_Url	:=	"https://api.umov.me/CenterWeb/api/"+AllTrim(c_key)+"/subGroup.xml"
	EndIf
	
	c_XML := ""
	c_XML += "data=<subGroup>" + CRLF
	c_XML += " <alternativeIdentifier>" + Alltrim(SBM->BM_GRUPO) + "</alternativeIdentifier>"+CRLF
	c_XML += " <description>" + Alltrim(AjuStr(SBM->BM_DESC)) + "</description>"+CRLF
	c_XML += " <active>" + IIf(lDel, "false", "true") + "</active>"+CRLF
	c_XML += "<group>
	// c_XML += "  <id>423993</id>
	// c_XML += "  <description>Material</description>
	c_XML += "  <alternativeIdentifier>GRPMAT</alternativeIdentifier>
	c_XML += "  <active>true</active>
	c_XML += "</group>
	c_XML += "</subGroup>" + CRLF
	...
	If ValType(s_PostRet) <> "U"
		...
		// SO-20091191 - Incluํda valida็ใo para nใo ocorrer error log - Gabriel Verํssimo 16/09/2020
		if Valtype(o_Xml) <> "U"
			c_IDMax := ""
			...
			c_Errors := "Falha no retorno do XML"
		endif
	Else
		If ValType(s_PostRet) == "U"
	...
	If !Empty(c_Errors)
		Conout("Nใo foi possํvel enviar o Grupo "+AllTrim(SBM->BM_GRUPO)+" a Max View. Mensagem do erro: "+AllTrim(c_Errors))
		If lMensagem
			MsgStop("Nใo foi possํvel enviar o Grupo "+AllTrim(SBM->BM_GRUPO)+" a Max View. Mensagem do erro: "+AllTrim(c_Errors))
		EndIf
		Return(Nil)
	EndIf
	...
	If !Empty(c_IDMax)
		DbSelectArea("SBM")
		SBM->(RecLock("SBM",.F.))
		SBM->BM_XMAXID := c_IDMax
		SBM->(MsUnlock())
	EndIf

	Conout("Grupo " + AllTrim(SBM->BM_GRUPO) + " enviado a Max View. "+Iif(!Empty(c_IDMax),"ID Max "+AllTrim(c_IDMax),""))
	If lMensagem
		MsgInfo("Grupo " + AllTrim(SBM->BM_GRUPO)+" enviado a Max View. "+Iif(!Empty(c_IDMax),"ID Max "+AllTrim(c_IDMax),""))
	EndIf
